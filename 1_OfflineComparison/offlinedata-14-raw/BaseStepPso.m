function [ParSwarm,OptSwarm,num_stop]=BaseStepPso(ParSwarm,OptSwarm,...
                                         mdltype,...
                                         AdaptFunc,ParticleScope,MaxW,MinW,LoopCount,CurCount,num_stop,num_stop_UpperBound)
...
% c1=0.35;c2=0.35;
% c1=0.5;c2=0.5;
c11=0.5;c12_0=0.5;c12_1=1.45;
c21=0.005;c22_0=0.005;c22_1=1.495;
%---------------------------------------------------------------------
%con=1;
...
%     TempV=c1*unifrnd(0,1).*SubTract1(row,:)+c2*unifrnd(0,1).*SubTract2;
    if num_stop<1000
        TempV(1:2)=c11*unifrnd(0,1).*SubTract1(row,1:2)+c12_0*unifrnd(0,1).*SubTract2(1:2);
        TempV(3:4)=c21*unifrnd(0,1).*SubTract1(row,3:4)+c22_0*unifrnd(0,1).*SubTract2(3:4);
    else
        TempV(1:2)=c11*unifrnd(0,1).*SubTract1(row,1:2)+c12_1*unifrnd(0,1).*SubTract2(1:2);
        TempV(3:4)=c21*unifrnd(0,1).*SubTract1(row,3:4)+c22_1*unifrnd(0,1).*SubTract2(3:4);       
    end
	%限制速度的代码
	for h=1:ParCol
...
% 	a=0.729;
	...
	%计算每个粒子的新的适应度值
    [acc,acc_train,score]=AdaptFunc(ParSwarm(row,1:ParCol),mdltype);
    ParSwarm(row,2*ParCol+1)=acc;
    ParSwarm(row,2*ParCol+2)=acc_train;
    ParSwarm(row,2*ParCol+3)=score;
	
    if (ParSwarm(row,2*ParCol+3) > OptSwarm(row,ParCol+3))
        OptSwarm(row,1:ParCol)=ParSwarm(row,1:ParCol);
        OptSwarm(row,ParCol+1:ParCol+3)=ParSwarm(row,2*ParCol+1:2*ParCol+3);
    end
    if OptSwarm(row,ParCol+3)>OptSwarm(ParRow+1,ParCol+3)
        OptSwarm(ParRow+1,:)=OptSwarm(row,:);
%         SwarmSize = ParRow;
%         ParticleSize = ParCol;
        ParSwarm(row,:)=rand(1,2*ParCol+3);
%         ParSwarm=rand(SwarmSize,2*ParticleSize+1);
        % adjust the location and velocity in particle swarm
        for k=1:ParCol
                ParSwarm(row,k)=ParSwarm(row,k)*(ParticleScope(k,2)-ParticleScope(k,1))+ParticleScope(k,1);
                  % adjust velocity, make the velocity and position correspond to the same range
                ParSwarm(row,ParCol+k)=ParSwarm(row,ParCol+k)*(ParticleScope(k,2)-ParticleScope(k,1))+ParticleScope(k,1);
        end        
        b=1;
    end
end
    if num_stop>num_stop_UpperBound
% %         SwarmSize = ParRow;
% %         ParticleSize = ParCol;
% %         ParSwarm(row,:)=rand(1,2*ParticleSize+1);
%         ParSwarm=rand(ParRow,2*ParCol+3);
%         % adjust the location and velocity in particle swarm
%         for k=1:ParCol
%                 ParSwarm(:,k)=ParSwarm(:,k)*(ParticleScope(k,2)-ParticleScope(k,1))+ParticleScope(k,1);
%                   % adjust velocity, make the velocity and position correspond to the same range
%                 ParSwarm(:,ParCol+k)=ParSwarm(:,ParCol+k)*(ParticleScope(k,2)-ParticleScope(k,1))+ParticleScope(k,1);
%         end        
%         b=1;
    end
    if b==0
