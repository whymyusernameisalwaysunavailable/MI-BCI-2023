...
%rand('state',0);
ParSwarm=rand(SwarmSize,2*ParticleSize+3);
...
for k=1:SwarmSize
        [acc,acc_train,score]=AdaptFunc(ParSwarm(k,1:ParticleSize),mdltype);
        ParSwarm(k,2*ParticleSize+1)=acc;
        ParSwarm(k,2*ParticleSize+2)=acc_train;
        ParSwarm(k,2*ParticleSize+3)=score;
end

%初始化粒子群最优解矩阵
OptSwarm=zeros(SwarmSize+1,ParticleSize+3);
%粒子群最优解矩阵全部设为零
[maxValue,row]=max(ParSwarm(:,2*ParticleSize+3));
%寻找适应度函数值最大的解在矩阵中的位置(行数)
OptSwarm(1:SwarmSize,1:ParticleSize)=ParSwarm(1:SwarmSize,1:ParticleSize);
OptSwarm(1:SwarmSize,ParticleSize+1:ParticleSize+3)=ParSwarm(1:SwarmSize,2*ParticleSize+1:2*ParticleSize+3);
